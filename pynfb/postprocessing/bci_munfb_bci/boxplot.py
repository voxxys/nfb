...
for subj, days in enumerate(desc['subjects'][:1]):
    for day, exp_name in enumerate(days[:1]):
        ...
        with h5py.File(exp_data_path) as f:
            ...
            df['block'] = df[['block_number', 'block_name']].apply(lambda x: ' '.join(map(str, x)), axis=1)
            print(df['block'].unique())
            df['block_type'] = df['block_name']
            df['block_type'].loc[df['block_number'] < 13] = df['block_type'][df['block_number'] < 13].apply(lambda x: x + ' BEFORE')
            df['block_type'].loc[df['block_number'] > 23] = df['block_type'][df['block_number'] > 23].apply(lambda x: x + ' AFTER')
            df['block_type'].loc[df['block_name'] == 'FB'] = df[['block_number', 'block_name']][df['block_name'] == 'FB'].apply(lambda x: ' '.join(map(str, x)), axis=1)
            del data

        blocks = ['FB', 'Baseline', 'Open', 'Left', 'Right']
        sns.boxplot('block_type', 'fb', 'block_name', data=df[df['block_name'].isin(blocks)], ax=axes[subj, day])
        axes[subj, day].legend_.remove()
